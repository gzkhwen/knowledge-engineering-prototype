          ...
          <List disablePadding>
            {menuItems.map((item) => {
              const selected = item.children
                ? item.children.some((child) => child.path === location.pathname || (child.path === "/admin/tool-hub" && /^\/admin\/tool-hub\/(?!run-records)/.test(location.pathname)))
                ...
                <Box key={item.path} sx={{ mb: 0.25 }}>
                  ...
                  {item.children?.map((child) => {
                    const childSelected = child.path === "/admin/tool-hub"
                      ? location.pathname === "/admin/tool-hub" || /^\/admin\/tool-hub\/(?!run-records)/.test(location.pathname)
